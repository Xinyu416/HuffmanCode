...
void ReadBMPFile() {
	...
	uint8_t* bgrcolors = (uint8_t*)malloc(info.biWidth * info.biHeight * 3);
	...
	printf("total:%d\n", sizeof(struct BGRCOLOR) * (info.biWidth * info.biHeight));
	...
	for (size_t i = 0; i < info.biHeight; i++)
	{
		...
		for (int j = 0; j < info.biWidth; j++) {
			...
			int pixel_index = (i * info.biWidth + j) * 3;
	...
	CollectData(bgrcolors, info.biWidth, info.biHeight, info.biWidth * info.biHeight * 3);
}
