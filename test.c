...
void Codec() {
	...
	//uint8_t* buffer = (uint8_t*)malloc(fileBytes);
	//size_t bytesRead = fread(buffer, 1, fileBytes, readStream);
	//printf("bytesRead count:%d\n", bytesRead);
	//for (uint32_t i = 0; i < fileBytes; i++)
	//{
	//	printf("data:%d ,", *(buffer + i));
	//}
	...
	//{
	//	//按8+1位取数据
	//	buf = BitPop(buffer, 8, offset);
	//	offset += 8;
	//	printf("\ndata: %d", buf);
	//	buf = BitPop(buffer, 1, offset);
	//	offset += 1;
	//	printf("\n---type:%d\n", buf);
	//}
	//free(buffer);
